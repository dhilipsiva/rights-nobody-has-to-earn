...
|---|---|---|---|---|
| computed from the constitution | **93** | **57** | **7583** | **10** |
...
Regenerated by `./generate.sh spine`, which reads the constitution directly. Do not edit
...
**State the method beside the base-predicate figure.** It lists *the engine's base predicates, minus
...
and written directly for most of the cast, while rules also produce it. The new
positive `family` and `parent` projections are mixed in the same way; supplied
legacy relationship facts remain valid. The prose vocabulary therefore includes
names the generated base-predicate list does not. They answer
...
`authority`'s original public-answerability route had a wholly negation-free
cone. The shared relation now also contains bounded constitutional powers;
**the generated block above is the current list and layer assignment**. This
file claimed until 2026-07-31 that `severe` had
...
**`obliged` does not add a chapter.** It derives from `err` and qualified
rights, records and findings; its current layer is in the generated block.
...
**The floor is inside personhood's dependency cone, not a disconnected layer.**
...
`prisoner`, `person`, and all eight floor predicates **inside the prisoner
cone**; the current layer numbers are generated above. The engine says so
directly: the refusal
...
1. **What Counts as Evidence.** What the world may report about a person, and nothing
   ...
   has to say so.
2. **Public Answerability, and Why It Is Never Revoked.** `authority`: the public
   answerability route is negation-free; the shared relation's current layer
   also reflects bounded constitutional powers. Two answerability derivations since
...
The chapter sequence is editorial rather than a strict ascending-stratum sort.
The generated table supplies the current layers of `defend`, `permits`,
`false` and `lose`; the expanding record families can move those layers.
...
`reward` and `become` remain here by editorial sequence,
not a claim that they float below the temporal
...
**Part IV — Consequence and accountability**
...
so the native spine freshness check cannot catch it if it goes stale — only the generated block
