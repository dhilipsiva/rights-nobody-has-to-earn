...
you like — that is the seventh — and separately, the state of your mind is not a thing
this society has any purchase on. There is no rule that reads *and if they think the
wrong thing*. The slot does not exist.

**Company** is on the floor, which is unusual enough to be worth defending. Solitude
imposed on a person is a harm, and a society that guaranteed food, shelter, care and
...
that harm. It must say which activity and which people it reaches, give public
reasons in a form people can actually use, carry a challenge that suspends it
while an independent body reviews, and carry an end. It may not be aimed at a
viewpoint, a belief, or who somebody is. The body doing the restricting cannot
be the body reviewing it, and neither of them can be the person who hears the
challenge.

Acting on a restriction is a separate step, and it has to point at the exact
one that was reviewed — the same matter, the same activity, the same version, the
same period, the same end. A restriction reviewed for one thing cannot be
carried over to another, and one reviewed for a newspaper cannot be moved to a
gallery. Acting on it also requires notice, reasons and a usable challenge
...
no meaning carried only by one sense or one notation. And inquiry and creation
are not directed: a researcher or an artist publishes or performs without prior
...
Equality is separate again, and it has its own chapter: the floor says what
nobody may be left without, and the chapter on the same route for everyone asks
whether a rule marks some people for a worse route to it.
...
has its own chapter, and so does the power to bind concentrated private power;
the whole of what either says here is that none of it touches the floor in
either direction.
