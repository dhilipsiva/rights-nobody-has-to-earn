...
Ruling D1 is complete. The review copy, the sample and a new summary edition
print the subtitle on the cover and carry it, with the promise, in the EPUB
and PDF metadata; every copy ends on a back cover that opens with the promise.
The book README and the proposal carry the subtitle, as the root README
already did.

Web artefacts leave print. The reference's closing "Continue to Chapter 1, or
return to the contents" is gone; the print layout hides the notes' "↩︎"
returns; and a link to a repository file, which paper cannot follow, prints its
path after it in print only. The chapters' pointers already print the
companion's address, and the method now prints the repository's, the site's and
the engine's guarantees document's. Cross-references are numeric: every
chapter title cited in prose or linked without a number gains "(Chapter N)",
Part V's joints are named as Chapter 29's, and a link to the method names the
method's section. One ledger needle follows its sentence.

Five diagrams, drawn by `tools/book_diagrams.py` as accessible SVG with a
title and description, stand in the back matter's diagrams section beside their
prose equivalents: the duty chain, delivery evidence, the placement ceiling,
the democratic corridor, and the six ways a fact is kept from a consequence.
The builder admits an image only from `book-1/diagrams/` with a text
alternative, inlines it in the HTML and PDF, packages it in the EPUB, and points
the companion's Markdown copy at the repository's raw file.

The works cited, `bibliography.md`, are generated by `tools/bibliography.py`
from the book's notes: each cited work once, alphabetical by first author's
surname or as written, with the places that cite it, further locators kept with
their work and notes that only point back ("The same judgment") adding their
place. An alphabetical index of 124 terms, bodies, people and documented cases,
generated by `tools/book_index.py` into a marked region at the end of
`reference.md`, lists each term's principal places: all of them for a rare
term, the eight that use a common one most, notes excluded. The manifest places
the works cited between the method and the reference, which the annotated
contents and the opening's navigation now mention. The summary edition,
`build_book.py --summary`, prints the companion's thirty-one plain-language
articles with the chapters that argue each, as a book of its own.

Checking the diagrams against the source corrected the companion's floor
note: bodily safety, expression and belief all have no delivery route, where it
had named two. The book-builder tests grow from nine to sixteen: diagrams are
admitted, inlined and packaged; a repository link prints its path in print
only; covers, back cover and metadata carry subtitle and promise; the print
copy has no web artefact and every image has alt text; the works cited and the
index are current, and every work a note cites reaches the list; and the
summary edition carries every article in order. The exact prose is
`session-drafted, author-approved under delegated approval (2026-09-13)`; the
reading-order decision records the item.

No rule, pin or suite changes, so item 62's complete run on the same formal
inputs stands. The rebuilt copies hold 40 ordered inputs; the review PDF has
256 pages, the sample 36 and the summary 16, and all three EPUBs pass
EPUBCheck 5.3.0, run through nix, with no errors or warnings. The PDFs carry no
return arrow or web closing, with subtitle first and promise last; the HTML
copies have no horizontal overflow at 360px or 1,280px, and the rendered
cover, back cover, diagram and note pages were inspected. All 149 authoring
development tests pass, with five declared ignored, as do the sixteen
book-builder, sixteen prose-lint and twelve companion input tests, the
companion's five library tests and the prose check; the companion exports 45
routes.
...
**Completed 2026-09-25 (item 64):** the subtitle is on every cover, in the EPUB
and PDF metadata, the READMEs and the proposal, and the promise on every back
cover.
...
- `book-1/contents.json` — the chapter manifest: front matter, parts, each
  ...
  (landed/planned), the back matter (`method.md`, then `bibliography.md`, the
  works cited generated from the notes, then `reference.md`, the map, glossary,
  diagrams and generated index), and the ordering rule. The exempt chapters form the last
