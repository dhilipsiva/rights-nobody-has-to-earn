...
sections below are inserted unchanged in `book-1/08-what-you-are-owed.md`: the
...
listed in the chapter's pin file.

Ecology, future generations and commons and Non-human animals were the last two
domains the reader ledger carried by a stated boundary alone. With these the set
is empty, and the ledger's own assertion is now an empty expectation, so a domain
landing without a failure passage fails the check rather than passing quietly.
