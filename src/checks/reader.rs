...
fn render_typed(source: &ReaderEvidenceSource, source_digest: &str) -> ReaderResult<String> {
    ...
    let mut lines = vec![
        ...
        "<!-- Generated by the native rights-verify reader-evidence refresh from reader-evidence.json. Do not edit. -->".to_owned(),
        String::new(),
    ...
    lines.extend([
        ...
        "- Regenerate: `./verify.sh --refresh reader-evidence`.".to_owned(),
        "- Structural/freshness check: `./verify.sh --quick`.".to_owned(),
        "- Authoritative executable contract controls: `./verify.sh`.".to_owned(),
        String::new(),
    ]);
    Ok(lines.join("\n"))
}
...
mod tests {
    ...
    fn typed_renderer_matches_the_reviewed_report_contract() {
        ...
        let rendered = render_typed(&source, &digest).unwrap();
        assert!(rendered.contains("Generated by the native rights-verify reader-evidence refresh"));
        assert!(rendered.contains("`./verify.sh --refresh reader-evidence`"));
        assert!(rendered.contains("`./verify.sh --quick`"));
        assert!(rendered.contains("Authoritative executable contract controls: `./verify.sh`"));
        assert!(!rendered.contains("python3 "));
        assert_eq!(rendered, render(&source_value, &digest).unwrap());
    }
