...
fn rules(cards: &[Card], beneficial: &BTreeSet<String>) -> Vec<String> {
    ...
    // Ruling D6: a record that only gives its subject something takes effect
    // on the source alone. Its duties follow at once, the named reviewer owes
    // prompt review and may withdraw them, and the completed record stays
    // behind full procedure, so nothing adverse is built on it.
...
/// duties and the prompt-review duty, never the completed record; for
